...
if [[ -n "${unhealthy:-}" ]]; then
    warn "some services still unhealthy after timeout:"
...
if [[ "${MATEHUB_OBSERVABILITY:-0}" == "1" ]]; then
    ...
    echo
    echo "  Both protected by basic auth at the Caddy edge:"
    printf '    user:     %s\n' "${OBSERVABILITY_USER:-admin}"
    printf '    password: %s%s%s\n' "$ANSI_YELLOW" "${OBSERVABILITY_PASSWORD:-(see deploy/.env)}" "$ANSI_RESET"
    echo
