...
#[allow(dead_code)] // will be used for transactional queries
    ...
    Fut: std::future::Future<Output = Result<T, sqlx::Error>>,
{
    ...
    // SET LOCAL only lasts for the current transaction, but we use it
    // on the connection level here. For non-transactional queries this
    // is fine -- the setting resets when the connection returns to the pool.
