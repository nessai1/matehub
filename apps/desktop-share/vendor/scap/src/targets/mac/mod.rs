...
pub fn get_all_targets() -> Vec<Target> {
    ...
    // MateHub patch: upstream unwraps here. `SCShareableContent::current`
    // returns Err when Screen Recording permission is not (yet) granted for
    // this exact code signature — unwrapping aborts the ENTIRE app (SIGABRT).
    // Degrade to an empty target list so the caller can surface the
    // permission prompt instead of crashing.
