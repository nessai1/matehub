...
pub(crate) fn create_capturer(
    ...
    // MateHub patch: honour Options.exclude_current_process_audio (upstream
    // scap declares the field but never wires it). Without this, the system
    // audio tap captures our OWN process output — the native voice playback —
    // and screen-share audio feeds back into an escalating loop.
    // SCStreamConfiguration.excludesCurrentProcessAudio, macOS 13+.
    stream_config.set_excludes_current_process_audio(options.exclude_current_process_audio);
